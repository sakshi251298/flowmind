...
The **Agentic Harness** is the intelligence layer of FlowMind, located in `apps/agent_service`. It is a sophisticated multi-agent orchestration system built on top of LangGraph and LangChain.
...
Before the agents begin their work, FlowMind can ingest vast amounts of custom context. When a user uploads a file or provides a link, the Go backend stores it, but the Python Agent Service actively parses it into the agent's memory.
