...
The React frontend for the FlowMind learning platform.
...
Generic UI primitives (Button, Dialog, Tabs, etc.) live in `packages/ui` as `@flowmind/ui`. Import them as:

```tsx
import { Button } from '@flowmind/ui/button'
import { Dialog, DialogContent } from '@flowmind/ui/dialog'
```
