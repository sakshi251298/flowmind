...
export CLUSTER_NAME=flowmind-prod
...
- database name: `flowmind`, or update the secret values below
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:secretsmanager:${AWS_REGION}:${AWS_ACCOUNT_ID}:secret:flowmind/prod/*"
    }
  ]
}
EOF
...
aws iam create-policy \
  --policy-name FlowMindExternalSecretsPolicy \
...
eksctl create iamserviceaccount \
  ...
  --role-name FlowMindExternalSecretsRole \
  --attach-policy-arn "arn:aws:iam::$AWS_ACCOUNT_ID:policy/FlowMindExternalSecretsPolicy" \
  --approve \
...
aws secretsmanager create-secret \
  --region "$AWS_REGION" \
  --name flowmind/prod/user-service \
  --secret-string '{
    "POSTGRES_URI": "postgresql://DB_USER:DB_PASSWORD@RDS_ENDPOINT:5432",
    "POSTGRES_DATABASE": "flowmind",
    "MIGRATE_DATABASE_URL": "postgres://DB_USER:DB_PASSWORD@RDS_ENDPOINT:5432/flowmind?sslmode=require"
  }'
```
...
aws secretsmanager create-secret \
  --region "$AWS_REGION" \
  --name flowmind/prod/agent-service \
  --secret-string '{
    "DATABASE_URL": "postgresql://DB_USER:DB_PASSWORD@RDS_ENDPOINT:5432/flowmind",
    "OPENAI_API_KEY": "",
    "ANTHROPIC_API_KEY": "",
    "GOOGLE_API_KEY": "",
    "OPENROUTER_API_KEY": "",
...
aws secretsmanager update-secret \
  --region "$AWS_REGION" \
  --secret-id flowmind/prod/agent-service \
...
```bash
kubectl -n flowmind rollout restart deployment/user-service deployment/agent-service
```
...
aws iam create-role \
  --role-name FlowMindGitHubDeployRole \
...
aws iam put-role-policy \
  --role-name FlowMindGitHubDeployRole \
  --policy-name FlowMindGitHubDeployPolicy \
...
```bash
export GITHUB_ROLE_ARN="arn:aws:iam::$AWS_ACCOUNT_ID:role/FlowMindGitHubDeployRole"
...
- `AWS_ROLE_TO_ASSUME`: `arn:aws:iam::<account-id>:role/FlowMindGitHubDeployRole`

Actions variables:

- `AWS_REGION`: for example `ap-south-1`
- `EKS_CLUSTER_NAME`: for example `flowmind-prod`
...
```bash
kubectl -n flowmind get pods
kubectl -n flowmind get externalsecrets
kubectl -n flowmind get ingress web
kubectl -n flowmind logs deployment/user-service
kubectl -n flowmind logs deployment/agent-service
```
...
- EKS cluster: `flowmind-prod`
- Kubernetes namespace: `flowmind`
- App domain: `app.example.com`
- Secrets Manager names: `flowmind/prod/user-service` and `flowmind/prod/agent-service`
- GitHub deploy role: `FlowMindGitHubDeployRole`
...
- Public subnets:
  - `kubernetes.io/cluster/flowmind-prod = shared`
...
5. Cluster IAM role: `FlowMindEKSClusterRole`.
...
4. Name: `general`.
5. Node IAM role: `FlowMindEKSNodeRole`.
...
4. DB instance identifier: `flowmind-prod`.
...
8. Connectivity:
   - VPC: same VPC as EKS.
   - Public access: **No**.
   - VPC security group: create `flowmind-rds`.
   - Initial database name: `flowmind`.
...
Create `flowmind/prod/user-service`:
...
3. Secret name: `flowmind/prod/agent-service`.
4. Choose **Store**.
...
4. Name: `FlowMindExternalSecretsPolicy`.
...
5. Attach `FlowMindExternalSecretsPolicy`.
6. Name: `FlowMindExternalSecretsRole`.
...
```bash
export EXTERNAL_SECRETS_ROLE_ARN="arn:aws:iam::$AWS_ACCOUNT_ID:role/FlowMindExternalSecretsRole"
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      "Effect": "Allow",
      "Action": "eks:DescribeCluster",
      "Resource": "arn:aws:eks:AWS_REGION:AWS_ACCOUNT_ID:cluster/flowmind-prod"
    }
  ]
}
```
...
3. Replace `AWS_REGION` and `AWS_ACCOUNT_ID`.
4. Name: `FlowMindGitHubDeployPolicy`.
5. Choose **Create policy**.
...
4. Audience: `sts.amazonaws.com`.
5. Attach `FlowMindGitHubDeployPolicy`.
6. Name: `FlowMindGitHubDeployRole`.
...
Open **Amazon EKS > Clusters > flowmind-prod > Access**.
...
2. IAM principal: `FlowMindGitHubDeployRole`.
3. Type: **Standard**.
...
- `EKS_CLUSTER_NAME = flowmind-prod`
...
In **Amazon EKS > Clusters > flowmind-prod > Resources**, check:
...
- ExternalSecret is not ready: check the `FlowMindExternalSecretsRole` trust policy `sub`, the service account annotation, and the secret names.
...
- GitHub Action can build but cannot deploy: check the EKS Access Entry for `FlowMindGitHubDeployRole`.
