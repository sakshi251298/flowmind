...
docker build -t flowmind-web:local -f apps/web/Dockerfile .

# User Service (Go)
docker build -t flowmind-user-service:local -f apps/user_service/Dockerfile apps/user_service

# User Service Migration
docker build -t flowmind-user-service-migrate:local -f apps/user_service/Dockerfile.migrate apps/user_service

# Agent Service (Python/Flask)
docker build -t flowmind-agent-service:local -f apps/agent_service/Dockerfile apps/agent_service
```
...
kubectl -n flowmind wait --for=condition=available deployment/postgres --timeout=120s
```
...
type: Opaque
stringData:
  POSTGRES_URI: "postgresql://flowmind:flowmind@postgres:5432"
  POSTGRES_DATABASE: "flowmind"
  MIGRATE_DATABASE_URL: "postgres://flowmind:flowmind@postgres:5432/flowmind?sslmode=disable"
---
apiVersion: v1
kind: Secret
metadata:
...
```bash
kubectl -n flowmind delete job user-service-migrate --ignore-not-found
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: migrate
          image: flowmind-user-service-migrate:local
...
kubectl -n flowmind wait job/user-service-migrate --for=condition=Complete --timeout=120s
```

Check migration logs:

```bash
kubectl -n flowmind logs job/user-service-migrate
```
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: user-service
          image: flowmind-user-service:local
          imagePullPolicy: Never
          ports:
            - name: http
              containerPort: 8080
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: agent-service
          image: flowmind-agent-service:local
          imagePullPolicy: Never
          ports:
            - name: http
              containerPort: 5001
...
kubectl -n flowmind rollout status deployment/user-service --timeout=120s
kubectl -n flowmind rollout status deployment/agent-service --timeout=120s
kubectl -n flowmind rollout status deployment/web --timeout=120s
```
...
```
127.0.0.1 flowmind.local
```

Open **http://flowmind.local** in your browser.

### Option B: Port-forward (simpler, no ingress needed)

```bash
kubectl -n flowmind port-forward svc/web 5173:80
```
...
```bash
minikube -n flowmind service web --url
```
...
# Services resolving?
kubectl -n flowmind get svc

# Secrets created?
kubectl -n flowmind get secrets

# Ingress assigned?
kubectl -n flowmind get ingress

# Health checks passing?
kubectl -n flowmind exec deploy/web -- curl -s http://localhost:8080/healthz
kubectl -n flowmind exec deploy/user-service -- wget -qO- http://localhost:8080/healthz
kubectl -n flowmind exec deploy/agent-service -- curl -s http://localhost:5001/api/health

# Logs for each service
kubectl -n flowmind logs deploy/web --tail=20
kubectl -n flowmind logs deploy/user-service --tail=20
kubectl -n flowmind logs deploy/agent-service --tail=20
```
...
kubectl -n flowmind rollout restart deployment/web
kubectl -n flowmind rollout status deployment/web --timeout=120s
```
...
| Pod `CrashLoopBackOff` | Check logs: `kubectl -n flowmind logs <pod-name> --previous` |
| Migration job fails | Check postgres is ready: `kubectl -n flowmind get pods -l app.kubernetes.io/name=postgres` |
